...
function printOrderv(orderId = null) {
	if(orderId) {		
			
		$.ajax({
			url: 'php_action/printOrderv.php',
			type: 'post',
			data: {orderId: orderId},
			dataType: 'text',
			success:function(response) {
...
function printOrderr(orderId = null) {
	if(orderId) {		
			
		$.ajax({
			url: 'php_action/printOrderr.php',
			type: 'post',
			data: {orderId: orderId},
			dataType: 'text',
			success:function(response) {
...
function printOrderc(orderId = null) {
	if(orderId) {		
			
		$.ajax({
			url: 'php_action/printOrderc.php',
			type: 'post',
			data: {orderId: orderId},
			dataType: 'text',
			success:function(response) {
		...
		success:function(response) {
			...
			var tr = '<tr id="row'+count+'" class="'+arrayNumber+'">'+			  				
				'<td>'+
					...
					'<select  class="form-control" name="productName'+count+'" id="productName'+count+'" onchange="getProductData('+count+');addRow();clxx()" >'+
				...
				'<td style="padding-left:20px;"">'+
					'<input type="text" name="rate'+count+'" id="rate'+count+'" autocomplete="off"  onkeyup="getTotal('+count+')"  class="form-control" />'+
					'<input type="hidden" name="rateValue[]" value="'+count+'" id="rateValue'+count+'" autocomplete="off" class="form-control" />'+
				'</td style="padding-left:20px;">'+
				'<td style="padding-left:20px;">'+
					'<div class="form-group">'+
					'<input type="number" name="quantity'+count+'" id="quantity'+count+'" onkeyup="getTotal('+count+')" autocomplete="off" class="form-control" min="1" />'+
					'</div>'+
				'</td>'+
				'<td style="padding-left:20px;">'+
				...
				'</td>'+
				'<td>'+
					'<button class="btn btn-default removeProductRowBtn" type="button" onclick="removeProductRow('+count+');clxy()"><i class="glyphicon glyphicon-trash"></i></button>'+
				'</td>'+
			'</tr>';
			if(tableLength > 0) {							
		...
		if(productId == "") {
			...
			$.ajax({
				...
				success:function(response) {
					...
					$("#rate"+row).val(response.prix);
					$("#rateValue"+row).val(response.prix);

					$("#quantity"+row).val(1);

					var total = Number(response.prix) * 1;
...
function printoverdevv(orderId = null) {
	if(orderId) {		
			
		$.ajax({
			url: 'php_action/printoverdevv.php',
			type: 'post',
			data: {orderId: orderId},
			dataType: 'text',
			success:function(response) {
